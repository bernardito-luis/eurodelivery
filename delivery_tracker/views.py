    ...
    if request.method == 'POST':
        if 'new_order_cancel' in request.POST:
            ...
            for i in range(1, max_prod_num+1):
                ...
                if request.POST.get('price_%d' % (i,)):
                    try:
                        foo = float(request.POST.get('price_%d' % (i,)))
                    except ValueError:
                        format_correct = False
                if request.POST.get('discount_in_shop_%d' % (i,)):
                    try:
                        foo = float(
...
def order_from_draft(request):
    ...
    if request.method == 'POST':
        ...
        old_order = get_object_or_404(PurchaseOrder, id=order_id)
        if request.user != old_order.user and not request.user.is_superuser:
        ...
        if 'new_order_cancel' in request.POST:
            ...
            new_purchase_order = old_order

            # delete all linked products
            Product.objects.filter(
                id__in=new_purchase_order.product_set.values_list(
                    'id', flat=True)
            ).delete()
            ...
            new_purchase_order.shipping_cost = (
                request.POST['shipping_cost'] or 0
            )
            new_purchase_order.coupon = request.POST.get('coupon') or ''
            new_purchase_order.discount = request.POST['discount'] or 0
            new_purchase_order.user_comment = (
                request.POST.get('user_comment') or ''
            )
            new_purchase_order.status = new_status
